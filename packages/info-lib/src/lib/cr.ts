...
export const getCrAvailableSemesters = async (helper: InfoHelper): Promise<CrSemester[]> => roamingWrapperWithMocks(
    ...
    async () => {
        ...
        return $("option").toArray().map((e) => ({
            id: (e as unknown as TagElement).attribs.value,
            name: ((e as unknown as TagElement).children[0] as TextElement).data?.trim(),
        } as CrSemester));
    },
    MOCK_AVAILABLE_SEMESTERS,
);
...
const getText = (e: Element, index: number) => {
    return cheerio((e as unknown as TagElement).children[index]).text().trim();
};
