...
import {platformFetchRaw} from "../utils/network";

/** cross-fetch 垫片（OneTHU 移植）：走平台传输，arrayBuffer 由 base64 解码 */
const fetch = async (url: string): Promise<{arrayBuffer: () => Promise<ArrayBuffer>}> => {
    const resp = await platformFetchRaw(url);
    const bin = atob(resp.text);
