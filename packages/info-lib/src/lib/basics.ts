    ...
    roamingWrapperWithMocks(
        ...
        () => uFetch(ASSESSMENT_LIST_URL).then((str) => {
            ...
            const result = cheerio("tbody", str)
                .children()
                .map((index, element) => {
                    const onclick = (((element as unknown as TagElement)
...
export const getClassroomState = (
    ...
    roamingWrapperWithMocks(
        ...
        () => uFetch(CLASSROOM_STATE_PREFIX + arbitraryEncode(building, "gb2312") + CLASSROOM_STATE_MIDDLE + week).then((s) => {
            ...
            const validWeekNumbers = $("#weeknumber option").map((_, element) => Number((element as unknown as TagElement).attribs.value)).get();
            ...
            const classroomStates = $("#scrollContent>table>tbody")
                .map((_, element) =>
                    (element as unknown as TagElement).children
                        .filter((it) => it.type === "tag" && it.tagName === "tr")
                        .map((tr) => {
                            const name =
                                ((tr as unknown as TagElement).children[1] as TagElement).children[2].data?.trim() ??
                                "";
                            const status = (tr as unknown as TagElement).children
                                .slice(3)
                                .filter((it) => it.type === "tag" && it.tagName === "td")
                                .map((td) => {
                                    const classNames =
                                        (td as unknown as TagElement).attribs.class?.split(" ")?.filter((it) => it !== "colBound") ?? [];
...
export const getBankPayment = async (
    ...
    roamingWrapperWithMocks(
        ...
        async (s) => {
            ...
            const options = cheerio("option", s).map((_, e) => (e as unknown as TagElement).attribs.value).get();
            if (options.length === 0) {
                return [];
            }
            ...
            const titles = $("div strong")
                .map((_, e) => {
                    const text = (e as unknown as TagElement).children[0].data?.trim();
                    if (text === undefined) {
                        return undefined;
                    }
