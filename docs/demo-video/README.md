...
## ★ 定稿现场流程（2026-09-22，模拟器单镜连续版）

> 真机分镜表（下文 §1）作废，以本节为准。录制设备：Mac 上的 Android 模拟器
> `onethu-demo`（Pixel 6 / 1080×2400 / AOSP android-35，M4 原生），scrcpy 录制，
> 状态栏演示模式与 IME 禁用在此镜像上全部生效。账号需**真实登录一次**（脱敏只换显示层）。

1. **打开导览**：设置 → 导览。
2. **过一遍导览**：全部按默认，一路「下一步」；账号步骤（雨课堂/OJ/邮箱/云盘）已配置的直接下一步，未配置的跳过此步。
3. **桌面小组件步骤**：按 HOME 退到桌面，**看一眼已放好的小组件**，回到应用继续。
4. **结束导览**（完成）。
5. **展示全部一级子页面**：按侧栏顺序逐页进入 → 静置 1.5–2s → 下一页。
6. **换个主题 → 芭比粉**。
7. **点 OH 胶囊**，问「今天有什么课」，等结构化答复卡片，静置 2.6s 收尾。
