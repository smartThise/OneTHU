...
- **侧栏 / 抽屉当前项指示条**（`.nav-indicator` + `useNavIndicator`）：**一条** WAAPI 采样动画
  驱动全部几何——`transform: translateY(条中心) scaleY(条长 / 16)`，位置与长度同属一条曲线，
  整条跑在合成器线程上（`transform` 是唯一可合成的几何通道）。起止都是静息条（16px、行内居中），
  前端点（下移=下端 / 上移=上端）全程一条**四次缓入缓出**曲线（峰值约 4× 平均速度，24 点采样）
  连续起停，尾端点延迟 30% 跟上，条自然先拉长再收回；**行程终点不是目标位，而是顺运动方向
  多滑出去的一点（惯性顶点）**：条带着末端速度**穿过**目标位、减速停在顶点，再由一帧
  ease-in-out 用 200ms 收回精确位置。顶点两侧速度都是 0，接缝连续——不会出现「到了-停住-弹簧」
  （实测穿过目标位那一刻速度 0.595 px/ms；旧结构「终点=目标位、另补一段从目标位滑到顶点」
  只有 0.002 px/ms，等于停在目标位）。滑行量 = 1/5 项高 × 路程系数，只滑一次、不来回震荡。
  滑行量系数**按路程递增**（位移折算行数：相邻项 ≈1 行 → 0，连滑行都不留、到位即停；
  隔两项 ≈3 行 → ≈0.25，约 1.6px；5 行以上 → 满额 1.0，约 6.4px），中间走二次曲线渐入；
  拉伸上限 3 个行高；时长按移动距离算 `clamp(140ms, 距离×1.2, 220ms)` + 200ms 收尾。
  CSS 里不再挂 transform transition（会与动画拼出速度突变）。
  挂在 `.nav`（滚动容器）里算内容坐标，滚动随内容走，内层 `.nav-folders-scroll` 也监听。
  量测用 rect，但**扣掉元素自身的动画位移**（`getComputedStyle().transform` 的 `m42`）：抽屉里的
  导航项有逐项入场 `m-rise`（`translate3d(0, 8px, 0)` 升起、`backwards` 填充），抽屉一开就量会
  按「项还在下面 8px」落位——条偏在项中间偏下，滑一下侧栏（scroll → place）才回正。同时监听
  `animationend` / `transitionend` 兜底重新对位。实测：布局位置 41px、动画中 rect 49px、扣掉后
  41px（偏差 8 → 0）。
  每项自己的 `::before` 强调条已退位。
  > **几何绝不要拆到两条动画 / 两条线程**（2026-09-24 事故，务必守住）：曾把「位置」放
  > `transform`（合成器线程）、「长度」放 `height`（主线程），两条动画各自计时。主线程被页面
  > 渲染占住时，位置继续走、长度卡在拉伸态，而条是**从顶端往下长**的——于是顶端已到目标、条却
  > 按自身长度往下多画出一大截，主线程缓过来再瞬间收回（霖反馈：长程回弹时小概率「弹出去很长
  > 一截，大部分是条自己的长度，后立刻收回」）。Edge 无头复现（长度动画冻在 96px、位置动画推到
  > 回弹结束）：条绘制区间 `[239, 334]`，比目标下端多出 **79px**；单动画同一时刻 `[239, 254]`，
  > 精确落在目标区间。`motion-test.mjs` 现有回归守卫：`bar.animate(frames)` 只能有一条，
  > 且源码里不许再出现 `bar.style.height`。
  > 已知取舍：`scaleY` 是非等比缩放，2px 圆角拉伸时按比例变长（静息态不变），理论上合成层的
  > 光栅化比例也会随拉伸倍数变化（曾有用户反馈「运动时稍微粗一点点」，霖本人未观察到）。Chromium
  > 实测（1x/2x、逐帧 16 帧含最大拉伸）绘制宽度恒为 3.00px，未复现该差异；若真机复现，应改用
  > `clip-path: inset(...)` 开窗（不缩放、仍是单一属性驱动），而不是再把几何拆回两条线程。
  > 踩过的坑：①两段 transition 各自从 0 起速 → 接缝处速度突变；②动画起点误用「整行框」
  > 而非 16px 静息条 → 每次切换条先蹦成一行长再缩回；③回弹窗 90ms + 回程 easeOutQuint：峰值处
  > 速度从 0 突跳成峰值，看着像「顿一下再弹回去」（霖反馈像卡了）——现两侧都是 ease-in-out；
  > ④位置与长度拆成两条动画（见上）→ 主线程卡住时条按自身长度探出去；⑤行程终点定在目标位、
  > 回弹另起一段 → 末端速度先归零再重新起步，观感「到了-停住-弹簧」（霖反馈：衔接不流畅，
  > 中间多了一段不合理的停顿）——现在把惯性顶点直接当行程终点；⑥量测撞上入场动画（见上）→
  > 条按动画中的位置落位，现在扣掉自身位移 + 动画结束兜底。
...
- **方阵网格**（其他 Info 服务 `.app-grid`、在线服务 `.thos-grid`）：纳入滚动揭示，**可见才入场**
  （小幅 `m-grid-in`：6px 上浮 + 0.99 缩放，220ms）。递延**按行分组**：同一行同时出现、行与行
  自上而下依次展开（行距 9ms、不封顶），读作"铺开"的流程感；视口外不再错过动画。
...
- **课表事件块**：入场按**行（时间带）**递延——行号 = 该块起始时刻在所有不同起始时刻里的序号，
  同一时刻跨天的块同时出现、时间从早到晚自上而下展开（13ms/行、16 行封顶；`m-rise` 220ms）。
  > 坑：`placed` 是「按天（列）再按时间」排序的，直接拿数组序号当延迟会变成竖直按列扫。`key` 里带周戳
