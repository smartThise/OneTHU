...
- **侧栏 / 抽屉当前项指示条**（`.nav-indicator` + `useNavIndicator`）：**一条** WAAPI 采样动画
  ...
  连续起停，尾端点延迟 30% 跟上，条自然先拉长再收回；到位后还有 **200ms 刹车回弹**（顺运动
  方向超出 1/5 项高，进出峰值两侧都 ease-in-out、峰值速度为 0——**只超出一次、不来回震荡**）；
  回弹强度**按路程递增**（位移折算行数：相邻项 ≈1 行 → 系数 0，连回弹窗都不挂、到位即停；
  隔两项 ≈3 行 → ≈0.25，约 1.6px；5 行以上 → 满额 1.0），中间走二次曲线渐入；
  拉伸上限 3 个行高；时长按移动距离算 `clamp(140ms, 距离×1.2, 220ms)`。CSS 里不再挂
  transform transition（会与动画拼出速度突变）。
  ...
  > ④位置与长度拆成两条动画（见上）→ 主线程卡住时条按自身长度探出去。
