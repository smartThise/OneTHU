...
- **侧栏 / 抽屉当前项指示条**（`.nav-indicator` + `useNavIndicator`）：**两条同步 WAAPI**
  采样动画——`moves`（transform: translateY = 条顶端）走合成器线程保证位置平滑，`sizes`
  （height = 条长度）在主线程只改这根绝对定位元素。起止都是静息条（16px、行内居中），
  ...
  拉伸上限 3 个行高；时长按移动距离算 `clamp(140ms, 距离×1.2, 220ms)`。只动 translateY 与
  height，**不做任何缩放**，CSS 里不再挂 transform transition（会与动画拼出速度突变）。
  挂在 `.nav`（滚动容器）里算内容坐标，滚动随内容走，内层 `.nav-folders-scroll` 也监听。
  每项自己的 `::before` 强调条已退位。
  > **为什么不用 `scaleY`**（2026-09-24 改）：非等比缩放会把 2px 圆角纵向拉成椭圆，并让合成层
  > 的光栅化比例随拉伸倍数变化——低分辨率设备上同一根条在静息/运动态的渲染宽度可能差出 1 个
  > 物理像素（用户反馈「运动时稍微粗一点点」）。改成 `height` 后，运动中的条与静息条是同一段
  > 3px 宽、2px 圆角矩形，只是更长。实测（Edge 无头 1x，逐帧采样 16 帧、含最大拉伸）：每帧
  > 绘制宽度都是精确 3.00px。
